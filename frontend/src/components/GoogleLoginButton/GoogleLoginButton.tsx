...
function loadGoogle(): Promise<GoogleIdentity> {
  ...
  scriptPromise = new Promise<GoogleIdentity>((resolve, reject) => {
    ...
    const fail = () => {
      ...
      reject(
        new Error(
          'Nie udało się załadować Google. Odśwież stronę lub użyj e-maila i hasła.',
        ),
      )
    }
...
function GoogleLoginButton({
            ...
            void loginWithGoogle(credential)
              ...
              .catch((error: unknown) => {
                if (
                  ...
                  error.code === 'google_link_confirmation_required'
                ) {
  ...
  return (
    <div className="google-login" aria-busy={busy}>
      <div className="google-login__separator">
        lub
      </div>
      ...
      {linkCredential && (
        <form
          ...
          onSubmit={(event) => {
            ...
            void loginWithGoogle(linkCredential, password)
              .then((response) => {
                if (!response.authenticated || !response.user) {
                  throw new Error(
